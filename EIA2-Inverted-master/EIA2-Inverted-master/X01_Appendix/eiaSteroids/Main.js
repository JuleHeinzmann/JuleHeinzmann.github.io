...
(function (eiaSteroids) {
    ...
    var frameTime = 1 / frameRate; // time per frame in seconds
    ...
    function shootProjectile(_origin, _target) {
        var velocity = eiaSteroids.Vector.getRandom(200, 200);
        if (_target) {
            velocity = eiaSteroids.Vector.getDifference(_target, _origin);
            velocity.scale(200 / velocity.length);
        }
        var projectile = new eiaSteroids.Projectile(_origin, velocity);
    ...
    function handleShipShot(_event) {
        var event = _event;
        var charge = event.detail.charge;
    ...
    function setHeading(_event) {
        var target = mapClientToCanvas(_event.clientX, _event.clientY);
        ship.head(target);
    }
    /**
    ...
    function shootLaser(_event) {
        var position = mapClientToCanvas(_event.clientX, _event.clientY);
        ship.shoot(position);
    }
    /**
    ...
    function breakAsteroid(_event) {
        ...
        if (asteroid.size > 0.3) {
            for (var i = 0; i < 2; i++) {
                var fragment = new eiaSteroids.Asteroid(asteroid.size / 2, asteroid.position);
    ...
    function createAsteroids(_nAsteroids) {
        ...
        for (var i = 0; i < _nAsteroids; i++) {
            var asteroid = new eiaSteroids.Asteroid(1.0);
     ...
     * Create a Ufo
     */
    function createUfo(_small) {
        if (_small === void 0) { _small = false; }
        console.log("Create ufo");
        var ufo = _small ? new eiaSteroids.UfoSmall : new eiaSteroids.Ufo();
        moveables.push(ufo);
    }
    /**
    ...
    function getColorCharge(_charge, _alpha) {
        ...
        var angle = 240 + 150 * _charge * _alpha;
        var light = 30 + 30 * _charge;
    ...
    function progress() {
        ...
        if (eiaSteroids.gamestate != GAMESTATE.PLAY)
            return;
        var difficulty = eiaSteroids.Info.score / 500;
